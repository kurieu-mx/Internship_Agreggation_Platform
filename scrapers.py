"""Fetching and parsing the upstream internship feed.
...
The rewrite reads the machine-readable ``listings.json`` that the same Pitt CSC
/ Simplify community repo publishes. It is a documented artifact of that repo
rather than an incidental detail of its presentation layer, so it is far less
prone to silent breakage — and it arrives already structured, which removes the
need to guess fields out of prose.
"""
